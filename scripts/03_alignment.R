...
fastq <- list.files("data/ChIP-seq/fastq",
                    pattern=".fastq.gz",
                    full.names=TRUE)
dir.create("data/ChIP-seq/BAM/") # Create directory for saving BAMs
...
align <- function(fastq_file) {
  ...
  line <- paste("bowtie2",
                ...
                "-S", gsub("fastq", "BAM", gsub(".fastq.gz", ".raw.sam", fastq_file))) # Output sam file name
  print(line)
  system(line)
} 

# 3) Run alignment for each fastq file
v <- sapply(fastq, align)
